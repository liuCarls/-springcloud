...
//@Order(2) //配置多个 HttpSecurity 实例，就像我们可以在xml文件中配置多个 <http>一样。关键在于多次扩展 WebSecurityConfigurationAdapter
...
//                .loginPage("/loginp.html")
                //默认成功url,如果不定义会是什么？
//                .defaultSuccessUrl("/welcome")
//                .failureUrl("/login-error.html")
//                .and().logout().logoutSuccessUrl("/loginp.html");
;

        http.authorizeRequests()
            .antMatchers("/css/**", "/js/**", "/fonts/**","/static/**").permitAll()  // 允许访问资源
            .withObjectPostProcessor(new ObjectPostProcessor<FilterSecurityInterceptor>() {
                @Override
                public <O extends FilterSecurityInterceptor> O postProcess(O o) {
                    o.setSecurityMetadataSource(myFilterInvocationSecurityMetadataSource);
...
//                out.flush();
//                out.close();
//                httpServletResponse.sendRedirect("/loginp");
                httpServletRequest.getRequestDispatcher("/loginp")
...
//                out.write(s);
//                out.flush();
//             out.close();//                httpServletResponse.setContentType("application/json;charset=utf-8");
//            }
...
//        http.formLogin().loginPage("/loginp.html").loginProcessingUrl("/login.do").defaultSuccessUrl("/indexp.html")//登录首页，默认验证通过后登录index.html，验证请求/login.do
//                .and().httpBasic()//开启httpBasic认证
//                .and().logout().logoutUrl("/logout.do").logoutSuccessUrl("/loginp.html")//登出后，跳转login.html
